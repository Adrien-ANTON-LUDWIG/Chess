...
namespace board
{
    ...
    void GameTracer::handle_castling(const PgnMove &move, const bool &color)
    {
        ...
        auto mv =
            Move(static_cast<Color>(color), move.get_start(), move.get_end());

        check_if_legal(mv, color);

        mv.execute_move(board_);
        check_state(move, color);
    ...
    void GameTracer::handle_move(const PgnMove &move, const bool &color)
    {
    ...
    void GameTracer::check_if_legal(const Move &mv, const bool &color)
    {
        auto legal_moves =
    ...
    void GameTracer::check_state(const PgnMove &move, const bool &color)
    {
    ...
    void GameTracer::check_checkmate(const PgnMove &move, const bool &color)
    {
    ...
    void GameTracer::check_check_and_stalemate(const PgnMove &move,
                                               const bool &color)
    {
        if (move.get_report() == ReportType::CHECK)
            for (auto l : listeners_)
    ...
    void GameTracer::play_pgn_move(PgnMove &move, bool &color)
    {
        if (move.get_piece() == PieceType::KING
            && abs(move.get_start().file_get() - move.get_end().file_get())
                == 2)
            handle_castling(move, color);
        else
            handle_move(move, color);

        history_.push_back(board_);

        check_checkmate(move, color);
        check_check_and_stalemate(move, color);
        check_draw();
